...
const updateGame = (p1, p2, gameState) => {
  ...
  if (p1.health < 0 || p2.health < 0) {
    ...
    console.log(`${p1.name}'s health is ${p1.health} and ${p2.name}'s health is ${p2.health}`)
    game.isOver = true
...
class Player {
  ...
  strike(player, enemy, attackDmg) {
    ...
    console.log(`${player.name} attacks ${enemy.name} for ${damageAmount} points`)
...
let player1 = new Player("Anu", 100, 15)
let player2 = new Player("Chikku", 100, 15)
...
// let click = document.getElementById("play")
...
play.onclick = () => {
  ...
  result.innerText = game.play(player1, player2)
}
...
document.addEventListener('keydown', function (e) {
  ...
  if (e.key == "q") {
    if (player2.health >= 0 && gameState == false) {
      i=0
      player1.strike(player1, player2, player2.attackDmg)
    }
...
document.addEventListener('keydown', function (e) {
  ...
  if (e.key == "a") {
    if (player1.health >= 0 && gameState == false) {
      i=0
...
document.addEventListener('keydown', function (e) {
  ...
  if (e.key == "p") {
    if (player1.health >= 0 && gameState == false) {
      i=0
      player2.strike(player2, player1, player1.attackDmg)
    }
